...
{
	{
		...
		auto cleanup = qScopeGuard([](){ qt_ntfs_permission_lookup--; });
		if (QFileInfo{program}.owner() == QFileInfo{QCoreApplication::applicationFilePath()}.owner())
			return false;
	}
	...
	auto isInAdminGroup = FALSE;
	if (!CheckTokenMembership(0, adminGroup, &isInAdminGroup))
		isInAdminGroup = FALSE;

	FreeSid(adminGroup);
	return !static_cast<bool>(isInAdminGroup);
}
